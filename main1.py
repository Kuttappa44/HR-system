...
# Response cache for faster retrieval
response_cache = {}
cache_ttl = 300  # 5 minutes cache TTL

# Pre-generated responses for instant delivery
PRE_GENERATED_RESPONSES = {
    "greeting": [
        "Hello! Thank you for taking the time to speak with me today.",
        "Hi there! I'm excited to learn more about you.",
        "Good to connect with you! Let's get started."
    ],
    "acknowledgment": [
    ...
    "follow_up": [
        ...
        "That's helpful! What else can you share?",
        "Interesting! Can you elaborate on that?",
        "Great! What's your experience with that?"
    ],
    "closing": [
...
async def handle_gather_response(call_sid: str, question_num: int, request: Request):
    ...
    try:
        ...
        # Generate AI response based on conversation (optimized)
        ai_response = await generate_ai_response(call_sid, speech_result, question_num)
        
        # Create optimized TwiML response
        response = VoiceResponse()
        
        # Use faster speech rate for quicker responses
        response.say(ai_response, voice='alice', rate='fast')
        ...
        if question_num < 3:  # Limit to 3-4 questions
            ...
            gather = Gather(
                input='speech',
                timeout=5,  # Reduced from 7 to 5 seconds
                action=f"{NGROK_URL}/gather-response/{call_sid}/{question_num + 1}",
                method='POST',
                speech_timeout='auto'  # Auto timeout for faster processing
            )
            response.append(gather)
            
            # Shorter fallback message
            response.say("Let me continue.", voice='alice', rate='fast')
            response.redirect(f"{NGROK_URL}/gather-response/{call_sid}/{question_num + 1}")
        else:
            # End the call with shorter message
            response.say("Thank you for your time. We'll be in touch soon.", voice='alice', rate='fast')
            response.hangup()
...
# Pre-defined response templates for faster responses
RESPONSE_TEMPLATES = {
    0: [  # First response templates
        "That's great to hear! What interests you most about this position?",
        "Wonderful! Can you tell me what draws you to this role?",
        "Excellent! What aspects of this job excite you the most?"
    ],
    1: [  # Second response templates
        "That's very interesting! Can you tell me about your relevant experience?",
        "Great! What experience do you have that relates to this position?",
        "Nice! Can you share some of your relevant work experience?"
    ],
    2: [  # Third response templates
        "That sounds impressive! What are you looking for in your next role?",
        "Excellent! What are your career goals for the next few years?",
        "Great! What would you like to achieve in your next position?"
    ],
    3: [  # Final response templates
        "Thank you for sharing that! Do you have any questions about the role?",
        "That's very helpful! Is there anything you'd like to know about the company?",
        "Perfect! Any questions you'd like to ask about the position or company?"
    ]
}
...
FALLBACK_RESPONSES = {
    ...
    "unclear_response": [
        "I want to make sure I understand correctly. Could you clarify that?",
        "Let me make sure I got that right. Can you explain a bit more?",
        "I want to understand better. Could you give me more details?"
    ],
    "generic": [
...
async def generate_ai_response(call_sid: str, user_response: str, question_num: int):
    """Generate AI response with optimized latency and caching"""
    import time
    ...
    try:
        ...
        # Quick fallback for very short or unclear responses
        if len(user_response.strip()) < 10:
            import random
            response_time = time.time() - start_time
            performance_metrics["response_times"].append(response_time)
            print(f"Fast fallback response in {response_time:.3f}s")
            return random.choice(FALLBACK_RESPONSES["short_response"])
        
        # Create cache key for similar responses
        cache_key = f"{question_num}_{len(user_response)}_{user_response[:20]}"
        cached_response = get_cached_response(cache_key)
        if cached_response:
            performance_metrics["cache_hits"] += 1
            response_time = time.time() - start_time
            performance_metrics["response_times"].append(response_time)
            print(f"Cache hit response in {response_time:.3f}s: {cached_response}")
            return cached_response
        
        # Use pre-defined templates for faster responses (80% of the time)
        import random
        if random.random() < 0.8:  # Increased to 80% for even faster responses
            performance_metrics["template_usage"] += 1
            templates = RESPONSE_TEMPLATES.get(question_num, RESPONSE_TEMPLATES[3])
            response = random.choice(templates)
            cache_response(cache_key, response)
            response_time = time.time() - start_time
            performance_metrics["response_times"].append(response_time)
            print(f"Template response in {response_time:.3f}s: {response}")
            return response
        
        # For 20% of responses, use AI generation with optimized parameters
        performance_metrics["ai_generation"] += 1
        responses = conversation.get("responses", [])
        
        # Build minimal context (only last response to reduce prompt size further)
        recent_response = responses[-1] if responses else ""
        context = f"Last: {recent_response}" if recent_response else ""
        
        # Ultra-optimized prompt - minimal and focused
        prompt = f"""HR interviewer. {context} Current: {user_response}. 
        Respond in 1 sentence. {"Ask follow-up." if question_num < 3 else "Wrap up."}"""
        
        # Optimized API call with fastest parameters
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Brief HR interviewer. 1 sentence max."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=30,  # Further reduced for faster generation
        ...
        response_time = time.time() - start_time
        performance_metrics["response_times"].append(response_time)
        print(f"AI generated response in {response_time:.3f}s: {ai_response}")
        return ai_response
        
    except Exception as e:
        print(f"Error generating AI response: {e}")
        # Return appropriate fallback based on question number
        import random
        templates = RESPONSE_TEMPLATES.get(question_num, RESPONSE_TEMPLATES[3])
        response_time = time.time() - start_time
        performance_metrics["response_times"].append(response_time)
...
async def get_performance_metrics():
    """Get performance metrics for AI response optimization"""
    import statistics
    
    if not performance_metrics["response_times"]:
        return {
    ...
    cache_hit_rate = (performance_metrics["cache_hits"] / total_requests * 100) if total_requests > 0 else 0
    template_usage_rate = (performance_metrics["template_usage"] / total_requests * 100) if total_requests > 0 else 0
    ai_generation_rate = (performance_metrics["ai_generation"] / total_requests * 100) if total_requests > 0 else 0
    ...
    return {
        ...
        "optimization_effectiveness": {
            "fast_responses_under_1s": len([t for t in response_times if t < 1.0]),
            "very_fast_responses_under_0_5s": len([t for t in response_times if t < 0.5]),
            "instant_responses_under_0_1s": len([t for t in response_times if t < 0.1])
        },
